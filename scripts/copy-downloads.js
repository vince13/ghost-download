...
const LANDING_DOWNLOADS = path.join(PROJECT_ROOT, 'landing', 'public', 'downloads');
...
console.log('📦 Copying Electron build files to landing/public/downloads/...\n');
...
if (!fs.existsSync(DIST_ELECTRON)) {
  ...
  console.error('   cd app && npm run electron:build');
  process.exit(1);
}
...
if (!fs.existsSync(LANDING_DOWNLOADS)) {
  ...
  console.log('📁 Created landing/public/downloads/ directory');
}
...
if (copiedCount === 0) {
  ...
  console.log(`\n✅ Done! Copied ${copiedCount} file(s) to landing/public/downloads/`);
  console.log('\n💡 Next steps:');
  console.log('   1. Commit the files: git add landing/public/downloads/');
